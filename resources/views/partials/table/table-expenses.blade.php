    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                ...
                <span x-text="filteredExpenses.length"></span> expenses
            </p>
        </div>
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-2 text-sm">
                <span class="text-gray-500 dark:text-gray-400">Total:</span>
                <span class="font-semibold text-gray-800 dark:text-white" x-text="formatCurrency(totalExpenses)"></span>
            </div>
            <div class="flex gap-2">
                <input type="text" x-model="searchTerm" @input.debounce.300ms="filterExpenses()" 
                    placeholder="Search expenses..." 
                    class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                <select x-model="filterCategory" @change="filterExpenses()" 
                    class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                ...
                <select x-model="filterStatus" @change="filterExpenses()" 
                    class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-200 dark:border-gray-700">
                    <th class="text-left py-2.5 px-3 text-gray-600 dark:text-gray-400 font-medium cursor-pointer" @click="sortBy('expense_date')">
                    ...
                    <th class="text-left py-2.5 px-3 text-gray-600 dark:text-gray-400 font-medium cursor-pointer" @click="sortBy('estate_name')">
                        <div class="flex items-center gap-1">
                            <span>Estate</span>
                            <span class="flex flex-col gap-0.5">
                    ...
                    <th class="text-left py-2.5 px-3 text-gray-600 dark:text-gray-400 font-medium">Payee</th>
                    <th class="text-left py-2.5 px-3 text-gray-600 dark:text-gray-400 font-medium">Category</th>
                    <th class="text-left py-2.5 px-3 text-gray-600 dark:text-gray-400 font-medium cursor-pointer" @click="sortBy('amount')">
                        <div class="flex items-center gap-1">
                            <span>Amount</span>
                            <span class="flex flex-col gap-0.5">
                    ...
                    <th class="text-left py-2.5 px-3 text-gray-600 dark:text-gray-400 font-medium">Description</th>
            ...
            <tbody>
                <template x-for="expense in paginatedExpenses" :key="expense.id">
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                        <td class="py-2.5 px-3 text-gray-600 dark:text-gray-400" x-text="formatDate(expense.expense_date)"></td>
                        <td class="py-2.5 px-3 text-gray-800 dark:text-white font-medium" x-text="expense.estate_name"></td>
                        <td class="py-2.5 px-3 text-gray-700 dark:text-gray-300" x-text="expense.payee_name"></td>
                        ...
                        <td class="py-2.5 px-3">
                            <span class="inline-flex px-2.5 py-1 text-xs rounded-full font-medium" 
                                :class="{
                                    'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400': expense.status === 'paid',
                                    'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400': expense.status === 'pending',
                                    'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400': expense.status === 'cancelled',
                                    'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400': expense.status !== 'paid' && expense.status !== 'pending' && expense.status !== 'cancelled'
                                }" 
                                x-text="expense.status ? expense.status.charAt(0).toUpperCase() + expense.status.slice(1) : 'Unknown'">
                            </span>
                        </td>
                    </tr>
                </template>
                ...
                <tr x-show="filteredExpenses.length === 0">
                    <td colspan="7" class="py-8 text-center text-gray-500 dark:text-gray-400">
                        ...
                        <p class="mt-1 text-sm" x-show="searchTerm || filterCategory || filterStatus">Try adjusting your filters</p>
                    </td>
                </tr>
            </tbody>
        </table>
