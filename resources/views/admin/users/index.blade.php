...
@php
    ...
    // Additional stats – role-based breakdown
    $staffCount = $users->filter(function($u) { return $u->role && !in_array($u->role->name, ['guest', 'tenant']); })->count();
    $tenantCount = $users->filter(function($u) { return $u->role && in_array($u->role->name, ['guest', 'tenant']); })->count();
    $verifiedCount = $users->filter(function($u) { return !is_null($u->email_verified_at); })->count();
...
<div class="container mx-auto px-4 py-6 max-w-screen-2xl">
    ...
    <div class="mb-4 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 p-3 border border-indigo-200 dark:border-indigo-800 flex flex-wrap items-center gap-2 text-sm">
        ...
        @endif
        <a href="{{ route('admin.users.index') }}" class="ml-auto text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium text-xs">
    ...
    <div class="mb-4 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
        <form id="filterForm" method="GET" action="{{ route('admin.users.index') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2">
            ...
            <div class="flex gap-1.5">
                ...
                <a href="{{ route('admin.users.index') }}" class="inline-flex items-center justify-center rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition h-9 w-9" title="Clear filters">
...
<!-- ============================================================ -->
<!-- ADD USER DRAWER (same as before, kept consistent) -->
<!-- ============================================================ -->
...
<!-- ============================================================ -->
<!-- MODALS (same as Staff but with indigo theme) -->
<!-- ============================================================ -->
...
<!-- ============================================================ -->
<!-- JAVASCRIPT (same functionality as Staff) -->
<!-- ============================================================ -->
<script>
    ...
    function exportSelectedUsers() {
        ...
        var ids = Array.from(selectedIds);
        var url = new URL('{{ route("admin.users.export") }}');
    ...
    function updateStats() {
        var url = new URL('{{ route("admin.users.index") }}');
